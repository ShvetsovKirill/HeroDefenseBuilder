...
## 7. Импорт в Unity — проверен

| что | состояние |
|---|---|
| пивоты | все модели в нуле координат |
| Animation Type | Generic |
| Scale Factor | 1, модели в метрах Blender |
| клипы | `KING_Idle` 1.97 с и `KING_Walk` 0.80 с — **Loop Time включён**; `KING_Attack` 0.77 с и `KING_Death` 1.30 с — без цикла |
| имена клипов | почищены от префикса `KING_rig\|` |
| текстуры | Filter Mode: Point, мипмапы включены, Wrap: Clamp, сжатие HQ |

Габарит воина после импорта: `min (-0.253, -0.022, -0.109)`, `max (0.304, 0.594, 0.089)`.
Низ на 2 см ниже нуля — это торчащее вниз древко топора, сама фигура стоит на земле.

Чего всё ещё нет: Animator Controller и префабов. Клипы лежат внутри FBX,
контроллер надо собрать под логику игры.

### Про пивоты — важная деталь

В первом экспорте воин уехал: `rig` стоял в сцене на X = 2.2, и в Unity вся иерархия
приходила со смещением `(-2.2, 0, 0)`. Здания экспортировались с явным обнулением, а воин —
нет. Исправлено, но правило общее: **перед экспортом обнулять положение корневого объекта**,
иначе смещение уезжает в FBX и всплывает уже в движке.
